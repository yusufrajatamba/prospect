...
### 2. Hybrid API Client Adapter ([`js/api.js`](js/api.js))
- Mendeteksi apakah URL Google Apps Script Web App telah diinput pengguna (`localStorage: agent_google_sheet_url`).
...
### 3. Antarmuka Pengaturan Database ([`index.html`](index.html) & [`css/style.css`](css/style.css))
...
### 4. Konfigurasi Vercel ([`vercel.json`](vercel.json))
...
### 5. Panduan Deployment Lengkap ([`CARA_DEPLOY_GOOGLE_SHEETS_DAN_VERCEL.md`](google_apps_script/CARA_DEPLOY_GOOGLE_SHEETS_DAN_VERCEL.md))
...
2. Ikuti instruksi di [`google_apps_script/CARA_DEPLOY_GOOGLE_SHEETS_DAN_VERCEL.md`](google_apps_script/CARA_DEPLOY_GOOGLE_SHEETS_DAN_VERCEL.md).
